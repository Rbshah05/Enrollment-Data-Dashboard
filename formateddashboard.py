...
# Shared variable to hold cleaned data
cleaned_df = None
...
with tab2:
    ...
    if 'cleaned_df' not in st.session_state:
        st.warning("Please upload a csv file tab first.")
    else:
                ...
                if not open_sections.empty:
                    ...
                    for idx, row in open_sections.iterrows():
                        with st.expander(f"{row['Descr']} — {row['Begin Time']} to {row['End Time']}"):
                            ...
                            with col1:
                                st.markdown(f"**Course Description:** {row['Descr']}")
                                st.markdown(f"**Start Time:** {row['Begin Time']}")
                                st.markdown(f"**End Time:** {row['End Time']}")

                            with col2:
                                st.markdown(f"**Instructors:** {row['Name']}")
                                st.markdown(f"**Total Enrolled:** {row['Tot Enrl']}")
                                st.markdown(f"**Enrollment Capacity:** {row['Enr Cpcty']}")
                else:
